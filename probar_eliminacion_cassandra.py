...
async def probar_eliminacion_corregida():
    ...
    try:
        ...
        fecha_prueba = "2025-12-15"  # Fecha diferente para no interferir con reserva existente
        ...
        print(f"🏠 Propiedades disponibles ANTES: {sorted(list(propiedades_antes))}")
        
        if 46 not in propiedades_antes:
            print("❌ La propiedad 46 no está disponible en esta fecha, no se puede probar")
            return
        ...
        print(f"🏠 Propiedades disponibles DESPUÉS: {sorted(list(propiedades_despues))}")
        ...
        if eliminadas == {46}:
            ...
            print(f"❌ Problema en eliminación. Esperado: {{46}}, Real: {eliminadas}")
