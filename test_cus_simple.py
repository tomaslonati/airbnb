...
async def test_cu_simple():
    ...
    try:
        ...
        if result.get('success'):
            data = result['data']
            print(f"   ✅ Ocupación calculada: {data.get('occupancy_rate', 0):.2f}%")
    ...
    try:
        ...
        print(f"   🚀 Mejora: {((time1-time2)/time1*100):.1f}%" if time1 > time2 else "Sin mejora")
    ...
    try:
        ...
        if community_result.get('success'):
            ...
            print(f"   🤝 Interacciones: {community.get('total_interactions', 0)}")
            print(f"   🏘️ Es comunidad: {community.get('is_community', False)}")
