        ...
        if hasattr(service, 'postgres_db') and service.postgres_db:
            ...
            try:
                async with service.postgres_db.pool.acquire() as conn:
                    ...
                    if rows:
                        for row in rows:
                            print(f"   💰 Precio: {row['precio_por_noche']}, Disponible: {row['disponible']}")
                    else:
                        print("⚠️ No hay registros en propiedad_disponibilidad para estas fechas")
                        ...
                        if prop_result:
                            print(f"🏠 Propiedad encontrada: ID {prop_result['id']}, Precio base: ${prop_result['precio_por_noche']}")
                        else:
