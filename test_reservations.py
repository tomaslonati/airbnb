...
async def test_reservation_system():
    ...
    try:
        ...
        print(
            f"✅ Login exitoso: {user_profile.nombre} (ID: {user_profile.huesped_id})")
        ...
        if availability_result.get("success") and availability_result.get("available"):
            print(
                f"✅ Propiedad {propiedad_id} disponible del {check_in} al {check_out}")
        else:
            print(
                f"❌ Propiedad {propiedad_id} NO disponible: {availability_result.get('message', 'Sin mensaje')}")
        ...
        if user_reservations.get("success"):
            ...
            print(
                f"❌ Error listando reservas: {user_reservations.get('error')}")
        ...
        if details_result.get("success"):
            ...
            print(
                f"❌ Error obteniendo detalles: {details_result.get('error')}")
        ...
        if availability_after.get("success") and availability_after.get("available"):
            print(
                f"⚠️ Las fechas aún aparecen como disponibles (puede estar bien si hay múltiples unidades)")
        else:
