...
class CUTester:
    ...
    async def test_cu1_ocupacion_por_ciudad(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.get('success'):
                ...
                print(f"   ✅ Tasa de ocupación: {occupancy_data.get('occupancy_rate', 0):.2f}%")
                print(f"   🏠 Propiedades totales: {occupancy_data.get('total_properties', 0)}")
                print(f"   📊 Días ocupados: {occupancy_data.get('occupied_days', 0)}")
    ...
    async def test_cu2_promedio_rating_anfitrion(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                if result:
                    promedio = float(result['promedio_rating']) if result['promedio_rating'] else 0.0
                    ...
                    print(f"   ⚠️ No hay reseñas para anfitrión {anfitrion_id}")
                    return {
                        "success": True,
                        "data": {
                            "anfitrion_id": anfitrion_id,
    ...
    async def test_cu3_propiedades_filtros(self) -> Dict[str, Any]:
        ...
        try:
            ...
            collection = cassandra_db.get_collection("properties_by_city_wifi_capacity")
            ...
            if properties:
                for i, prop in enumerate(properties[:3]):  # Mostrar solo las primeras 3
    ...
    async def test_cu4_disponibles_fecha(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                if results:
                    for i, prop in enumerate(results[:5]):  # Mostrar solo las primeras 5
    ...
    async def test_cu5_reservas_fecha_ciudad(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Buscar en Cassandra
            cassandra_db = self.services['cassandra']
            collection = cassandra_db.get_collection("reservas_por_ciudad_fecha")
    ...
    async def test_cu8_cache_busqueda(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"   📊 Mejora: {((time1 - time2) / time1 * 100):.1f}%" if time1 > time2 else "No hay mejora")
    ...
    async def test_cu9_usuarios_recurrentes(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.get('success'):
                ...
                print(f"   ✅ Usuario recurrente detectado: {analysis['is_recurrent']}")
                print(f"   📊 Total reservas: {analysis['total_bookings']}")
                print(f"   🏙️ Ciudades únicas: {analysis['unique_cities']}")
                print(f"   🔄 Reservas recurrentes: {analysis['recurrent_bookings']}")
    ...
    async def test_cu10_comunidades_interacciones(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.get('success'):
                ...
                print(f"   🤝 Total interacciones: {community_data.get('total_interactions', 0)}")
                print(f"   🏘️ Es comunidad: {community_data.get('is_community', False)}")
    ...
    async def print_summary(self):
        ...
        for test_name, result in self.results.items():
            ...
            print(f"{status} {test_name}: {'ÉXITO' if result.get('success') else 'FALLO'}")
