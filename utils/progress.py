...
class ProgressBar:
    ...
    def _display(self):
        ...
        filled = int(self.width * self.current /
                     self.total) if self.total > 0 else self.width
        bar = "█" * filled + "░" * (self.width - filled)

        elapsed_time = time.time() - self.start_time

        print(
            f"\r{self.prefix}: [{bar}] {percent:.1f}% ({self.current}/{self.total}) - {elapsed_time:.1f}s", end="", flush=True)
